...
std::vector<std::pair<int, std::string>> board_info = {
    {1, "/dev/ttyAMA0"}, {2, "/dev/ttyAMA2"}, {3, "/dev/ttyAMA1"}, {4, "/dev/ttyAMA3"}};
    ...
    std::thread capture_thread(
        [utc_time_str]()
        {
            try
            {
                ...
                SQLite::Database db(g_config.db_file, SQLite::OPEN_READWRITE);
...
void load_dots_and_center(SQLite::Database& db)
{
    ...
    try
    {
        ...
        while (query.executeStep())
        {
            ...
            cout << "[DEBUG] Loaded dot pair: " << id1 << "<->" << id2 << " (" << p1.x << "," << p1.y << ") <-> ("
                 << p2.x << "," << p2.y << ")" << endl;
        }
        ...
        {
            Point inter;
            bool found = calculate_intersection(get<1>(base_line_pairs[0]), get<3>(base_line_pairs[0]),
                                                get<1>(base_line_pairs[1]), get<3>(base_line_pairs[1]), inter);
            if (found)
            {
                dot_center = inter;
            }
...
void load_rule_lines(SQLite::Database& db)
{
    ...
    try
    {
        ...
        while (query.executeStep())
        {
            ...
            cout << "[DEBUG] Loaded line (scaled): " << line.name << " [(" << line.start.x << "," << line.start.y
...
{
    return block.find("<tt:VideoAnalytics>") != string::npos && block.find("<tt:Frame") != string::npos;
}
...
bool is_linecrossing_event(const string& event_block, string& object_id, string& rule_name)
{
    ...
    if (!regex_search(event_block, topic_match, topic_regex) ||
        string(topic_match[1]).find("LineCrossing") == string::npos)
    {
        return false;
    }
...
void analyze_risk_and_alert(SQLite::Database& db, int human_id, const string& rule_name, const string& utc_time_str)
{
    ...
    Point line_vector = {crossed_line.end.x - crossed_line.start.x, crossed_line.end.y - crossed_line.start.y};
    ...
    for (const auto& [vehicle_id, vehicle_state] : vehicle_trajectory_history)
    {
        ...
        cout << "[DEBUG] Vehicle " << vehicle_id << ": Closest dot = (" << closest_dot.x << "," << closest_dot.y << ")"
             << endl;
        cout << "[DEBUG] Oldest Pos = (" << oldest_pos.x << "," << oldest_pos.y << "), Newest Pos = (" << newest_pos.x
             << "," << newest_pos.y << ")" << endl;
        ...
        cout << "[DEBUG] Vehicle " << vehicle_id << ": Old dist to dot_center = " << dist_old
             << ", New dist = " << dist_new << endl;

        if (dist_new > dist_old - g_config.dist_threshold)
        {
            cout << "[DEBUG] Step Failed (Vehicle " << vehicle_id
                 << "): Not approaching dot_center enough. Threshold = " << g_config.dist_threshold << endl;
            continue;
        }
        ...
        cout << "[DEBUG] Vehicle " << vehicle_id << ": Cosine similarity = " << similarity
             << ", Threshold = " << g_config.parallelism_threshold << endl;

        if (abs(similarity) >= g_config.parallelism_threshold)
        {
            cout << "\n[ALERT] " << vehicle_id << " 차량이 " << human_id << " 인간을 향해 측면에서 접근 중입니다."
                 << endl;
        ...
        if (xml_buffer.find("</tt:MetadataStream>") != string::npos)
        {
...

            {
                ...
                if (is_linecrossing_event(block, object_id, rule_name))
                {
                    ...
                    {
                        // cout << "[DEBUG] A line was crossed by object " << object_id << ", but it was not identified
                        // as a human in recent frames." << endl;
                    }
                }
            }
        }
...
int main()
{
    ...
    try
    {
        ...
        //     cerr << "[FATAL] Failed to load configuration from database. Check 'baseLines' and 'lines' tables." <<
        //     endl; return 1;
        // }
