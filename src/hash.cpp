 ...
 * @details 이 파일은 Argon2id 알고리즘을 사용하여 비밀번호와 복구 코드를 해싱하고 검증하는 기능을 제공합니다. 또한 복구
 * 코드를 생성하는 기능도 포함합니다.
 */
...
bool verify_password(const string& hashed_password, const string& password)
{
    ...
    int result = crypto_pwhash_str_verify(hashed_password.c_str(), password.c_str(), password.length());
