...
int main()
{
    ...
    while (true)
    {
        ...
        if (action == 1)
        {
            ...
            if (!sendAll(ssl, reinterpret_cast<const char*>(&len), sizeof(len)))
                break;
            if (!sendAll(ssl, step1_out.c_str(), step1_out.size()))
                break;
            ...
            if (!recvAll(ssl, buf.data(), res_len))
                break;
            ...
            if (step1_res.value("step1_success", 0) == 1)
            {
                ...
                if (requires_otp == 1)
                {
                    ...
                    if (!sendAll(ssl, reinterpret_cast<const char*>(&len), sizeof(len)))
                        break;
                    if (!sendAll(ssl, step2_out.c_str(), step2_out.size()))
                        break;
        ...
        else if (action == 2)
        {
            ...
            if (crypto_pwhash_str(client_hash, passwd.c_str(), passwd.size(), crypto_pwhash_OPSLIMIT_INTERACTIVE,
            ...
            if (!sendAll(ssl, reinterpret_cast<const char*>(&len), sizeof(len)))
                break;
            if (!sendAll(ssl, signup_out.c_str(), signup_out.size()))
                break;
